...
Public Class CLHIEIBUS
    ...
    'Public Overrides Function GetClifor(ByVal strTipoCF As String, ByVal strDitta As String, ByRef dttOut As DataTable, ByVal strFiltraConAgenti As String, ByVal strWhere As String) As Boolean
    '    Dim strSQL As String = ""
    '    Try

    '        If oApp.BUSNET_VERSIONDB < 19 Then
    '            strSQL = ApexNetLIB.EmbeddedResource.GetString(GetType(CLDIEIBUS).Assembly, "GetCliforOld.sql")
    '        Else
    ...
    '        strSQL = ApexNetLIB.EmbeddedResource.GetString(GetType(CLHIEIBUS).Assembly, "GetClifor.sql")
    '        'TNET_FINEMO
    ...
    Public Overrides Function GetCliforAge(ByVal strDitta As String, ByRef dttOut As DataTable, ByVal strWhere As String) As Boolean
        Dim strSQL As String = ""
        Try
            ...
            'TNET_INIMOD
            strSQL = ApexNetLIB.EmbeddedResource.GetString(GetType(CLHIEIBUS).Assembly, "GetCliForAge.sql")
            strSQL = strSQL.Replace("@ditta", CStrSQL(strDitta))

            'sostituisce chiavi con valori lette da registro Business
            Dim pZONAGE_ZONCOD As String, pZONAGE_AGECOD As String, pZONAGE_AGEDES As String
            pZONAGE_ZONCOD = GetSettingBusDitt(strDitta, "Bsieibus", "Opzioni", ".", "@ZONAGE_ZONCOD", "", " ", "-1").Trim
            pZONAGE_AGECOD = GetSettingBusDitt(strDitta, "Bsieibus", "Opzioni", ".", "@ZONAGE_AGECOD", "", " ", "-1").Trim
            pZONAGE_AGEDES = GetSettingBusDitt(strDitta, "Bsieibus", "Opzioni", ".", "@ZONAGE_AGEDES", "", " ", "'(DIREZIONALE)'").Trim

            strSQL = strSQL.Replace("@ZONAGE_ZONCOD", pZONAGE_ZONCOD)
            strSQL = strSQL.Replace("@ZONAGE_AGECOD", pZONAGE_AGECOD)
            strSQL = strSQL.Replace("@ZONAGE_AGEDES", pZONAGE_AGEDES)
...
#Region "Personalizzazioni Apex"
    ...
    'Sostituito da  InsertLeadNoteData(
    'Public Overrides Function InsertLeadNote(ByVal strDitta As String, ByVal dRow As DataRow, ByRef CodLead As Integer) As Boolean
    ...
    '        CodLead = NTSCInt(dRow("COD_LEAD").ToString)
    ...
    '        vtb_desnota = dRow("TIPO_NOTA").ToString
    '        If vtb_desnota.Trim = "" Then
    '            vtb_desnota = "Nota creata da IB, operatore: " & dRow("COD_OPERATORE").ToString.Trim & ", il: " & _
    '                                NTSCDate(dRow("DAT_INSERT").ToString).ToString("dd/MM/yyyy").ToString
    '            vtb_desnota = Left(vtb_desnota, 80)
    '        End If

    '        nNumero = AggNuma(strDitta, "NT", " ", 0, nNumero, False, False, "", dbConn)
    '        strSQL = "insert into tabnote ( codditt, tb_codnote, tb_desnote, tb_tiponot, tb_contonot, tb_testonot) values  ( {0}, {1}, {2}, 'S', {3}, {4} ) "
    '        strSQL = String.Format(strSQL, CStrSQL(strDitta), nNumero, CStrSQL(vtb_desnota), CodLead, CStrSQL(dRow("NOTA").ToString))
    ...
    ' Procedura di Esempio di come usare il web service al posto del file XML
    ...
    'Sostituito da InsertLeadData(
    'Public Overrides Function InsertLead(ByVal strDitta As String, ByVal dRow As DataRow, ByRef CodLead As Integer) As Boolean
    '    Dim strSQL As String = ""
    ...
    '            If Not dRow("DESCRIZIONE1").ToString Is Nothing Then
    '                vRagSoc = Left(dRow("DESCRIZIONE1").ToString, 70).Trim
    '                If vRagSoc <> "" Then
    '                    vRagSoc = Replace(Replace(Replace(Replace(Replace(Replace(Replace(LTrim(RTrim(vRagSoc)), " ", ""), ".", ""), "-", ""), "\", ""), "/", ""), "+", ""), ",", "")
    '                End If
    '            End If
    '            If Not dRow("INDIRIZZO").ToString Is Nothing Then
    '                vIndir = Left(dRow("INDIRIZZO").ToString, 60).Trim
    '                If vIndir <> "" Then
    '                    vIndir = Replace(Replace(Replace(Replace(Replace(Replace(Replace(LTrim(RTrim(vIndir)), " ", ""), ".", ""), "-", ""), "\", ""), "/", ""), "+", ""), ",", "")
    '                End If
    '            End If
    '            If Not dRow("TELEFONO").ToString Is Nothing Then
    '                vTelef = Left(dRow("TELEFONO").ToString, 18).Trim
    '                If vTelef <> "" Then
    '                    vTelef = Replace(Replace(Replace(Replace(Replace(Replace(Replace(LTrim(RTrim(vTelef)), " ", ""), ".", ""), "-", ""), "\", ""), "/", ""), "+", ""), ",", "")
    '                End If
    '            End If
    ...
    '                'Se LEAD esistente INSERISCE LEAD NELLA LISTA SELEZIONATA Della campagna e alimento le eventuali NOTE
    '                CodLead = NTSCInt(dtl.Rows(0)!le_codlead)
    '                bRet = Lead_Aggiorna_Dati(strDitta, dRow, CodLead)
    '                Return bRet
    ...
    '            strSQL = "SELECT  TOP 1  * " & vbCrLf & _
    '                        " FROM  TabStat " & vbCrLf & _
    '                        " WHERE     LTrim(RTrim(tb_desstat)) = " & CStrSQL((dRow("NAZIONE").ToString.Trim)) & vbCrLf
    ...
    '            'TNET_MODIMAN
    '            'Azioni aggiuntive al NUOVO LEADS
    '            v_CampCod = NTSCInt(dRow("COD_CAMPAGNA"))

    '            If dRow("COD_OPERATORE").ToString.Trim.ToLower.Contains("gloria") Then
    '                codAgente = "999"
    '            Else
    '                codAgente = NTSCInt(dRow("COD_AGENTE")).ToString
    '                If NTSCInt(codAgente) = 0 Then
    '                    'se non trova l'agente impostato, mette colombo
    '                    codAgente = "12"
    '                End If
    ...
    '                        "'S'," & _
    '                        CodLead & "," & _
    '                        CStrSQL(NTSCStr(dRow("DES_NOTE"))) & "," & _
    '                        "GETDATE() "
    '            Execute(strSQL, CLE__APP.DBTIPO.DBAZI)
    '            vtb_codnote = AggNuma(strDitta, "NT", " ", 0, vtb_codnote, False, False, "")
    '            '--------------
    ...
    '                        "FROM operat " & _
    '                        "WHERE opiscrmus = 'S' " & _
    '                            " AND OpNome <>	" & CStrSQL(dRow("COD_OPERATORE"))
    '            v_operatori_crm = OpenRecordset(strSQL, CLE__APP.DBTIPO.DBPRC)
    ...
    '                                            CodLead & "," & vbCrLf & _
    '                                            "'L', " & vbCrLf & _
    '                                            CStrSQL(NTSCStr(dRow("DES_NOTE"))) & vbCrLf & _
    '                                    "   )   " & vbCrLf
    '                            Execute(strSQL, CLE__APP.DBTIPO.DBAZI)
    ...
    Public Overridable Function Lead_Aggiorna_Dati(ByVal strDitta As String, ByVal dRow As DataRow, ByRef CodLead As Integer) As Boolean
                        ...
                        If dt3.Rows.Count > 0 Then
                            ...
                            If dtl.Rows.Count > 0 Then
                                ...
                                strSQL = "UPDATE listsel  SET  " & vbCrLf & _
                                        "       lse_note = " & CStrSQL(NTSCStr(dRow("DES_NOTE"))) & vbCrLf & _
                                        " WHERE " & vbCrLf & _
